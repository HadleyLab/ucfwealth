...
data:
  ...
  APP_INIT_URL: `echo -n http://covidimaging-backend-$TIER-aidbox:$AIDBOX_PORT | base64 -w 0`
  APP_OVERRIDE_AIDBOX_BASE_URL: `echo -n http://covidimaging-backend-$TIER-aidbox:$AIDBOX_PORT | base64 -w 0`
  APP_PORT: `echo -n $AIO_PORT | base64 -w 0`
  APP_SECRET: `echo -n $APP_SECRET | base64 -w 0`
  APP_URL: `echo -n http://covidimaging-backend-$TIER-auto-deploy:$AIO_PORT | base64 -w 0`
  BACKEND_PUBLIC_URL: `echo -n $AIDBOX_BASE_URL | base64 -w 0`
  FRONTEND_URL: `echo -n $FRONTEND_URL | base64 -w 0`
kind: Secret
metadata:
  name: covidimaging-backend-$TIER-app
